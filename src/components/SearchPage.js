...
const SearchPage = () => {
  ...
  let [editShortCut, setEditShortCut] = useState();
  ...
  let [signUpControl, isShortCutOpen, editShortCutOpen] = useSelector((state) => {
    return [state.searchReducer.signUpControl, state.searchReducer.isShortCutOpen, state.searchReducer.editShortCutOpen];
  })
  let dispatch = useDispatch();

  const getShortCuts = () => {
    getDocs(query(collection(database, `users/${auth.currentUser.uid}/shortCuts`), orderBy('dateAdded', 'asc')))
      .then((snapshot) => {
        let shortCuts = [];
  ...
  const getEditShortCut = (editShortCutOpen) => {
    getDoc(doc(database, `users/${auth.currentUser.uid}/shortCuts/${editShortCutOpen.id}`))
      .then((snapshot) => {
        setEditShortCut({
          ...snapshot.data(),
          id: snapshot.id
  ...
  useEffect(() => {
    getShortCuts();
    if (editShortCutOpen) {
      getEditShortCut(editShortCutOpen);
    }
  }, [editShortCutOpen]);
  ...
  const addShortCutFunc = () => {
    if (shortCutName && shortCutURL) {
      ...
      toast.dark('Please, enter name and url correctly!');
    }
  }

  const editShortCutFunc = () => {
    if (shortCutName && shortCutURL) {
      setDoc(doc(database, `users/${auth.currentUser.uid}/shortCuts/${editShortCut.id}`), {
        name: shortCutName,
        url: shortCutURL,
        dateAdded: new Date().getTime()
      })
  ...
  if (searchParams.get('q')) {
    ...
    return (
      <div>
                  ...
                  <div style={{ position: "fixed", top: "50%", left: "50%", backdropFilter: "brightness(0.5)", width: "100%", height: "100vh", transform: "translate(-50%, -50%)", zIndex: "100" }}>
                    <div style={{ position: "absolute", top: "50%", left: "50%", background: "#fff", transform: "translate(-50%, -50%)", width: "500px", padding: "14px" }}>
                      ...
                      <form style={{ margin: "10px 0" }}>
                        ...
                        }} label="Name" variant="filled" sx={{ mb: "14px" }} placeholder={editShortCut ? editShortCut.name : ''} />
                        <TextField size='small' fullWidth id="filled-basic" onChange={(e) => {
                          setShortCutURL(e.target.value);
                        }} label="URL" variant="filled" placeholder={editShortCut ? editShortCut.url : ''} />
                        <Box sx={{ textAlign: "end", my: "14px" }}>
                          ...
                          <Button size="small" variant='outlined' onClick={editShortCut ? editShortCutFunc : addShortCutFunc} sx={{ fontSize: "12px" }}>Add</Button>
                        </Box>
                      </form>
                    </div>
                  </div>
        ...
        {/* customize chrome */}
        <div style={{ padding: "0 10px", textAlign: "end", boxSizing: "border-box" }}>
          <button className='shadow' style={{ padding: "5px 20px", fontSize: "12px", borderRadius: "30px", border: "none", color: "blue", background: "transparent" }}><i className="fa-solid fa-pen"></i> Customize Chrome</button>
        </div>
      </div>
    )
  }
